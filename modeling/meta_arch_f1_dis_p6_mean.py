...
class ClipRCNNWithClipBackbone(GeneralizedRCNN):
    ...
    def __init__(self, cfg) -> None:
        ...
        self.pro = Singe_prototype(1024, 6)
        self.di = invariant()
        self.ds = specific()
...
class ClipRCNNWithClipBackboneWithOffsetGenTrainable(ClipRCNNWithClipBackbone):
    ...
    def forward(self, batched_inputs):
        ...
        if np.random.rand(1) > self.apply_aug:
            ...
            content_mean, content_std = calc_mean_std(base_ds)
            base_ds = (base_ds - content_mean.expand(
                base_ds.size())) / content_std.expand(base_ds.size())
            base_ds = base_ds * std.expand(base_ds.size()) + mean.expand(
                base_ds.size())
            base_ds = self.relu(base_ds)
    ...
    def opt_offsets(self, batched_inputs):
        ...
        for i, val in enumerate(self.domain_tk.items()):
            ...
            content_mean, content_std = calc_mean_std(features['res4'])
            content_feat_norm = (features['res4'] - content_mean.expand(
                features['res4'].size())) / content_std.expand(features['res4'].size())
            aug_feat = content_feat_norm * self.stylestd[i - 1].expand(features['res4'].size()) + self.stylemean[
                i - 1].expand(features['res4'].size())
            aug_feat = self.relu(aug_feat)
