...
class ClipRCNNWithClipBackbone(GeneralizedRCNN):
    ...
    def inference(
        ...
        # cv2.imshow("res4 Feature Map", res4_feature_normalized)
        #
...
class ClipRCNNWithClipBackboneWithOffsetGenTrainable(ClipRCNNWithClipBackbone):
    ...
    def __init__(self, cfg) -> None:
        ...
        # print("f1_dis_p6")
        with open('prunedprompts1.txt', 'r') as f:
            for ind, l in enumerate(f):
                # print("i:", ind)
                # print("l:", l)
                domain_text.update({str(ind): l.strip()})
        # for key, value in domain_text.items():
        #     print(f"Key: {key}, Value: {value}") #包含Key: day, Value: an image taken during the day
        ...
        # for key, value in self.domain_tk.items():
    ...
    def forward(self, batched_inputs):
        ...
        with torch.no_grad():
            ...
            # 可视化 res4 特征图
        res4_feature_numpy = res4_feature.squeeze().cpu().detach().numpy()
        res4_feature_normalized = (res4_feature_numpy - res4_feature_numpy.min()) / (
                res4_feature_numpy.max() - res4_feature_numpy.min())
        plt.imsave('/home/zzh/domaingen/res4_feature_map.png', res4_feature_normalized, cmap='viridis')
    ...
    def opt_offsets(self, batched_inputs):
        ...
        for i, val in enumerate(self.domain_tk.items()):
            name, dtk = val
            # print(f"Index: {i}, Key: {name}, Value: {dtk}")
            # print(name)
            # print(dtk)
            if name == 'day':
                 continue
