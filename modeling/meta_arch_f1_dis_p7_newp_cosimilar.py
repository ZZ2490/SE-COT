...
class ClipRCNNWithClipBackbone(GeneralizedRCNN):
    ...
    def __init__(self, cfg) -> None:
        ...
        self.di = invariant()
        self.ds = specific()
        self.pro2 = Singe_prototype(2048, 7)
    ...
    def inference(
        ...
        classprototype = self.pro2(self.backbone.forward_res5(features['res4']))
        classprototype = F.interpolate(classprototype, size=base_di.shape[-2:], mode='bilinear', align_corners=False)
        # 手动调整通道数

        classprototype = classprototype[:, :base_di.shape[1], :, :]
        # for i in range(20):
        ...
        features['res4'] = self.pro(features['res4']) + classprototype
...
class ClipRCNNWithClipBackboneWithOffsetGenTrainable(ClipRCNNWithClipBackbone):
    ...
    def forward(self, batched_inputs):
        ...
        classprototype = F.interpolate(classprototype, size=base_di.shape[-2:], mode='bilinear', align_corners=False)
        classprototype = classprototype[:, :base_di.shape[1], :, :]
        # prototypeloss = (1 - torch.cosine_similarity(classprototype, base_di, dim=1)).mean()
        ...
        # loss_di2 = {'prototypeloss': prototypeloss}
        losses = {}
        losses.update(loss_di)
        # losses.update(loss_di2)
        losses.update(detector_losses)
        losses.update(proposal_losses)
        return losses
